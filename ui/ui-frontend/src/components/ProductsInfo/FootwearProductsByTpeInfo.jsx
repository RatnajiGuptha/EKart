...
const FootwearProductsByTypeInfo = () => {
  ...
  const Inventory = "footwear";
  const InventoryType = `footwearBy/${type}`;

  const handleWishListItem = async () => {

    if (localStorage.getItem('token')) {
      const datad = await WishListService.getItemByTypeAndId(Inventory, footWearId).then()
        .catch((err) => {
          if (err.response.status === 401) {
            console.log(err.response.data)
            navigate("/login")
      ...
      if (datad.data.wishlistId == null) {
        const wishList = {
          ...
          prodId: productsInfo.footWearId,
          logoImg: productsInfo.logoImg,
          type: productsInfo.type,
          productName: productsInfo.productName,
